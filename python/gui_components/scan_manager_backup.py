...
class ScanManager:
    ...
    def on_scan_button_click(self):
        ...
        if self._validate_scan_inputs(source_path, profile_name, destination_root):
            ...
            return        # Clear previous preview results
        if hasattr(self.app, 'preview_tree'):
            self.app.preview_tree.clear()  # PyQt5 method to clear all items        self.result_queue = Queue()
        ...
        # Use QTimer instead of tkinter's after method
        self.check_timer = QTimer()
        self.check_timer.timeout.connect(self._check_scan_queue)
    ...
    def refresh_scan_data(self):
        ...
        self.app.status_manager.complete_validation_stage()        # Clear previous data from treeviews
        if hasattr(self.app, 'source_tree'):
            self.app.source_tree.clear()  # PyQt5 method
        if hasattr(self.app, 'preview_tree'): 
            self.app.preview_tree.clear()  # PyQt5 method

        self.app.refresh_btn.setEnabled(False)  # Disable button during scan (PyQt5)

        self.result_queue = Queue() 
        
        thread_args = (source_path, profile_name, destination_root)
        print(f"DEBUG: _refresh_scan_data: Starting thread with args: {thread_args}", flush=True)        self.scan_thread = threading.Thread(target=self._scan_worker, args=thread_args, daemon=True)
        self.scan_thread.start()        # Use QTimer instead of tkinter's after method
        self.check_timer = QTimer()
        self.check_timer.timeout.connect(self._check_scan_queue)
        self.check_timer.start(100)  # Check every 100ms
        """Validate scan inputs and show appropriate errors."""
        if not source_path or not os.path.isdir(source_path):
            self.app.status_label.setText("Error: Please select a valid source folder.")  # PyQt5
            print("Scan aborted: No valid source folder selected.", flush=True)
            return False
        if not profile_name:
            self.app.status_label.setText("Error: Please select a profile.")  # PyQt5
            print("Scan aborted: No profile selected.", flush=True)
            return False
        if not destination_root:
            self.app.status_label.setText("Error: Please select a destination folder.")  # PyQt5
            print("Scan aborted: No destination folder selected.", flush=True)
            return False
        
        if not self.app.normalizer:
            self.app.status_label.setText("Error: Normalizer not available. Check logs.")  # PyQt5
    ...
    def _scan_worker(self, source_path: str, profile_name: str, destination_root: str):
        ...
        try:
            ...
            self.result_queue.put({"type": "final_error", "data": error_message})    def _check_scan_queue(self):
        ...
        try:
            ...
            # Stop the timer and re-enable refresh button
            if hasattr(self, 'check_timer'):
                self.check_timer.stop()
            self.app.refresh_btn.setEnabled(True)  # PyQt5

            if isinstance(result, dict) and result.get("type") == "final_error":
                error_data = result.get('data', 'Unknown error during scan.')
                self.app.status_label.setText(f"Scan Failed: {error_data}")  # PyQt5
                ...
                self.app.status_label.setText(f"Scan Complete. Generated {num_proposals} proposals from {base_name}.")  # PyQt5
                ...
                self.app.status_label.setText("Scan finished with unexpected result.")  # PyQt5
            ...
            # Continue checking with QTimer instead of tkinter's after
            ...
            if hasattr(self, 'check_timer'):
                self.check_timer.stop()
            self.app.refresh_btn.setEnabled(True)  # Re-enable refresh button on error too (PyQt5)
            error_msg = f"Error processing scan result: {e}\n{traceback.format_exc()}"
            self.app.status_label.setText(error_msg)  # PyQt5
            print(error_msg)
    ...
    def _schedule_general_adapter_processing(self, status_data):
        ...
        # Use QTimer for PyQt5 instead of tkinter's after
        QTimer.singleShot(0, lambda: self.app.status_manager.process_adapter_status(status_data))
