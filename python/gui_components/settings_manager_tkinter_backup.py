...
class SettingsManager:
    ...
    def get_current_ui_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "ffplay_path": getattr(self.app, 'ffplay_path_var', tk.StringVar()).get() if hasattr(self.app, 'ffplay_path_var') else ""
            }
        except Exception as e:
    ...
    def save_ui_state(self, key: str, value: Any):
        """Save a specific UI state property.
        
        Args:
            key: The UI state property name
            value: The value to save
        """
        try:
            settings = self.load_settings()
            if "ui_state" not in settings:
                settings["ui_state"] = self.default_settings["ui_state"].copy()
            
            settings["ui_state"][key] = value
            self.save_settings(settings)
            print(f"Saved UI state: {key} = {value}")
        except Exception as e:
            print(f"Error saving UI state {key}: {e}")